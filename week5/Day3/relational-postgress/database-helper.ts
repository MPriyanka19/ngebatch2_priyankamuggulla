...
export const getEmployees = async (Request: Request, response: Response) => {
  ...
  export const insertEmployee = async (Request :Request , Response : Response)=>{
    ...
    try{
      ...
      Response.json(' Employee inserted');
    }
    catch (error) {
        console.log('Error thrown in inserting the employee: ', (error as Error).message)
        Response.status(500).json({ message: 'There was an error' })
      }
  ...
  export const updateEmployee = async (
    ...
    try {
      const emp = await pool.query(
        `update employee set age= 25 where empno=${eno};`
      );
      console.log("Employees = ", emp);
      Response.json(`Employee updated ${eno}`);
    } catch (error) {
      console.log(
        "Error thrown in updating the employee: ",
