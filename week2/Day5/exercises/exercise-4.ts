...
const findFavouriteFood = async (name:string)=>{
  try{
  const a = await queryUser(name)
  const b = await queryFood(a.food)
  return `${a.name} likes ${b.name}`
  }
  catch(err){
    return ("Not found")
  }
...
//
findFavouriteFood('John').then(console.log) //Test rejection for not providing a name
findFavouriteFood('Megan').then(console.log) //Test rejection for giving a name that isn't in database
