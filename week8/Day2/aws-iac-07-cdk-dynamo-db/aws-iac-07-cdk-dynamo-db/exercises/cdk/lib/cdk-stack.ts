...
export class CdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: GigsSettings) {
    ...
    // gig data GET lambda
    const getGigDataLambda = new nodejs.NodejsFunction(
      this,
      "get-gig-data-lambda",
      {
        functionName: `${props.subDomain}-get-gig-data-lambda`,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: "./functions/dynamo-lambdas.ts",
        handler: "getGigDataHandler",
        environment: lambdaEnvVars,
        timeout: cdk.Duration.seconds(30),
        bundling,
      }
    );
    // gig data by id GET lambda
    const getGigDataByIdLambda = new nodejs.NodejsFunction(
      this,
      "get-gig-data-by-id-lambda",
      {
        functionName: `${props.subDomain}-get-gig-data-by-id-lambda`,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: "./functions/dynamo-lambdas.ts",
        handler: "getGigDataByIdHandler",
        environment: lambdaEnvVars,
        timeout: cdk.Duration.seconds(30),
        bundling,
      }
    );
    // bootstrap all gig data lambda
    const bootstrapGigDataLambda = new nodejs.NodejsFunction(
      this,
      "bootstrap-gig-data-lambda",
      {
        functionName: `${props.subDomain}-bootstrap-gig-data-lambda`,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: "./functions/dynamo-lambdas.ts",
        handler: "bootstrapGigDataHandler",
        environment: lambdaEnvVars,
        timeout: cdk.Duration.seconds(30),
        bundling,
      }
    );
    // gig data POST lambda
    const postGigDataLambda = new nodejs.NodejsFunction(
      this,
      "post-gig-data-lambda",
      {
        functionName: `${props.subDomain}-post-gig-data-lambda`,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: "./functions/dynamo-lambdas.ts",
        handler: "gigDataPostHandler",
        environment: lambdaEnvVars,
        timeout: cdk.Duration.seconds(30),
        bundling,
      }
    ...
    // API GW.
    // Bootstrap & Put Dynamo Items should *not* be available in the API!
    ...
    // gig-data GET and POST
    const gigDataApi = api.root.addResource("gigdata");
    gigDataApi.addMethod(
      "GET",
      new apigw.LambdaIntegration(getGigDataLambda, { proxy: true })
    );
    gigDataApi.addMethod(
      "POST",
      new apigw.LambdaIntegration(postGigDataLambda, { proxy: true })
    );
    // gig-data-by-id GET
    const gigDataByIdApi = api.root.addResource("gigdatabyid"); // makes '/api/gigdatabyid'
    const gigIdParameter = gigDataByIdApi.addResource("{gig_id}"); // makes '/api/gigdatabyid/123'
    gigIdParameter.addMethod(
      "GET",
      new apigw.LambdaIntegration(getGigDataByIdLambda, { proxy: true })
    );
